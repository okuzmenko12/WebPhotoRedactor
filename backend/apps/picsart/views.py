...
from .service import PCsService, get_count_of_enhances_for_field
...
class BaseImageAPIView(UserSubscriptionsService, APIView):
    ...
    counter_enhance_type: Optional[str] = None
    ...
    def post(self, *args, **kwargs):
        ...
        if not user.is_authenticated:
            ...
            if self.user_have_active_subscriptions(user):
                ...
                if get_count_of_enhances_for_field(user, self.counter_enhance_type) == 0:
...
class UpscaleAPIVIew(BaseImageAPIView):
    ...
    counter_enhance_type = CounterModelEnhanceFields.up_scales_count


class RemoveBGAPIView(BaseImageAPIView):
    enhance_type = ImageEnhanceTypes.remove_bg
    counter_enhance_type = CounterModelEnhanceFields.bg_deletions_count


class RemoveJPEGArtifactsAPIView(BaseImageAPIView):
    enhance_type = ImageEnhanceTypes.remove_jpeg_artifacts
    counter_enhance_type = CounterModelEnhanceFields.jpg_artifacts_deletions_count
